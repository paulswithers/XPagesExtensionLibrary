...
public class JdbcFunctions extends FBSDefaultObject {
	...
	public JdbcFunctions(JSContext jsContext) {
		...
		addFunction(FCT_GETCONNECTION, "@JdbcGetConnection", "(data:T):Ljava.sql.Connection;"); // $NON-NLS-1$
																								// $NON-NLS-2$
		addFunction(FCT_DBCOLUMN, "@JdbcDbColumn", "(connection:Wtable:Tcolumn:T):A", "(connection:Wtable:Tcolumn:Twhere:T):A", "(connection:Wtable:Tcolumn:Twhere:TorderBy:T):A",
				"(connection:Wtable:Tcolumn:Twhere:TorderBy:Tparams:A):A"); // $NON-NLS-1$
																			// $NON-NLS-2$
																			// $NON-NLS-3$
																			// $NON-NLS-4$
																			// $NON-NLS-5$
		addFunction(FCT_EXECUTEQUERY, "@JdbcExecuteQuery", "(connection:Wsql:T):Ljava.sql.ResultSet;", "(connection:Wsql:Tparams:A):Ljava.sql.ResultSet;"); // $NON-NLS-1$
																																							// $NON-NLS-2$
																																							// $NON-NLS-3$
		addFunction(FCT_INSERT, "@JdbcInsert", "(connection:Wtable:Tvalues:W):I", "(connection:Wtable:Tvalues:WcolumnNames:A):I"); // $NON-NLS-1$
																																	// $NON-NLS-2$
																																	// $NON-NLS-3$
		addFunction(FCT_UPDATE, "@JdbcUpdate", "(connection:Wtable:Tvalues:W):I", "(connection:Wtable:Tvalues:Wwhere:T):I", "(connection:Wtable:Tvalues:Wwhere:Tparams:A):I"); // $NON-NLS-1$
																																												// $NON-NLS-2$
																																												// $NON-NLS-3$
																																												// $NON-NLS-4$
		...
		public FBSValue call(IExecutionContext context, FBSValueVector args, FBSObject _this) throws JavaScriptException {
			...
			try {
				...
				case FCT_GETCONNECTION: {
					if (args.size() >= 1) {
						final String name = args.get(0).stringValue();
				...
				case FCT_DBCOLUMN: {
					if (args.size() >= 3) {
						final Connection c = getConnection(args.get(0));
						final String tbName = args.get(1).stringValue();
						final String colName = args.get(2).stringValue();
						final String where = args.size() > 3 && !args.get(3).isNull() ? args.get(3).stringValue() : null;
						final String orderBy = args.size() > 4 && !args.get(4).isNull() ? args.get(4).stringValue() : null;
						final FBSValue params = args.size() > 5 && !args.get(5).isNull() ? args.get(5) : null;
						...
						try {
							...
							try {
								final ArrayObject result = new ArrayObject(getJSContext());
								while (rs.next()) {
				...
				case FCT_EXECUTEQUERY: {
					if (args.size() >= 2) {
						final Connection c = getConnection(args.get(0));
						final String sql = args.get(1).stringValue();
						final FBSValue params = args.size() > 2 && !args.get(2).isNull() ? args.get(2) : null;
				...
				case FCT_INSERT: {
					final FBSValue retVal_ = doInsert(args, context, true);
					if (null != retVal_) {
						return retVal_;
					}
				}
				...
				case FCT_INSERT_EXT: {
					final FBSValue retVal_ = doInsert(args, context, false);
					if (null != retVal_) {
						return retVal_;
					}
				}
				...
				case FCT_UPDATE: {
					final FBSValue retVal_ = doUpdate(args, context, true);
					if (null != retVal_) {
						return retVal_;
					}
				}
				...
				case FCT_UPDATE_EXT: {
					final FBSValue retVal_ = doUpdate(args, context, false);
					if (null != retVal_) {
						return retVal_;
					}
				}
				...
				case FCT_DELETE: {
					if (args.size() >= 2) {
						final Connection c = getConnection(args.get(0));
						final String tbName = args.get(1).stringValue();
						final String where = args.size() >= 2 && !args.get(2).isNull() ? args.get(2).stringValue() : null;
						final FBSValue params = args.size() >= 3 && !args.get(3).isNull() ? args.get(3) : null;
	...
	protected static void initParameters(PreparedStatement st, FBSValue params, int offset) throws SQLException, InterpretException {
		if (params != null) {
			final int count = params.getArrayLength();
			for (int i = 0; i < count; i++) {
				final FBSValue v = params.getArrayValue(i);
				final Object o = v.toJavaObject();
	...
	private static class SQLValues {
		...
		SQLValues(FBSValue params) throws SQLException, InterpretException {
			if (params instanceof ObjectObject) {
				final FBSDefaultObject o = (FBSDefaultObject) params;
				namedValues = new HashMap<String, Object>();
				for (final Iterator<JSProperty> it = o.getPropertyIterator(); it.hasNext();) {
					final JSProperty p = it.next();
					final String name = p.getPropertyName();
					final Object value = p.getValue().toJavaObject();
					namedValues.put(name, value);
				}
			} else if (params instanceof ArrayObject) {
				final ArrayObject o = (ArrayObject) params;
				final int n = o.getArrayLength();
				values = new ArrayList<Object>(n);
				for (int i = 0; i < n; i++) {
					final FBSValue v = o.getArrayValue(i);
					values.add(v.toJavaObject());
				}
			} else {
				final Object o = params.toJavaObject();
				if (o != null) {
					if (o instanceof Map) {
						namedValues = (Map) o;
	...
	protected static List<Object> initInsertValues(StringBuilder b, FBSValue params, Boolean uCase) throws SQLException, InterpretException {
		final SQLValues sqlValues = new SQLValues(params);
		// In case of an array
		...
		if (sqlValues.namedValues != null) {
			...
			b.append(" (");
			for (final Map.Entry<String, Object> e : sqlValues.namedValues.entrySet()) {
				if (!first) {
					b.append(',');
				} else {
					first = false;
	...
	protected static FBSValue doInsert(FBSValueVector args, IExecutionContext context, Boolean uCase) throws SQLException, InterpretException {
		if (args.size() >= 2) {
			final Connection c = getConnection(args.get(0));
			final String tbName = args.get(1).stringValue();
			final FBSValue values = args.get(2);
			final FBSValue idColumnNames = args.size() > 3 && !args.get(3).isNull() ? args.get(3) : null;
			;

			final StringBuilder b = new StringBuilder();
			b.append("INSERT INTO "); // $NON-NLS-1$
			JdbcUtil.appendTableName(b, tbName);
			final List<Object> v = initInsertValues(b, values, uCase);
			...
			if (idColumnNames == null) {
				...
				final List<Object> vNames = initInsertValues(null, idColumnNames, uCase);
				final String[] columnNames = new String[vNames.size()];
			...
			try {
				...
				if (idColumnNames != null) {
					...
					if (rs.next()) {
						final Object value = rs.getBigDecimal(1);
	...
	public static FBSValue doUpdate(FBSValueVector args, IExecutionContext context, Boolean uCase) throws SQLException, InterpretException {
		if (args.size() >= 3) {
			...
			final FBSValue values = args.get(2);
			final String where = args.size() >= 3 && !args.get(3).isNull() ? args.get(3).stringValue() : null;
			final FBSValue params = args.size() >= 4 && !args.get(4).isNull() ? args.get(4) : null;

			final StringBuilder b = new StringBuilder();
			b.append("UPDATE "); // $NON-NLS-1$
			JdbcUtil.appendTableName(b, tbName);
			final List<Object> v = initUpdateValues(b, values, uCase);
			if (StringUtil.isNotEmpty(where)) {
				b.append(" WHERE "); // $NON-NLS-1$
				b.append(where);
			}
